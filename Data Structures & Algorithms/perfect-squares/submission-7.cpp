    ...
    //    }
     ...
     int numSquares(int n) {
      ...
      while(!q.empty()) {
         ...
         //if(t.first > n) continue; //not needed because we are checking before inserting
         if(t.first == n) return t.second; //goal
         
         for(int i = 1; i <= sqrt(n); i++) {
            int next = t.first + i*i; 
            if(next<=n && !visited[next]) { //check next<=n first to avoid accessing visited out of bound
            visited[next] = true;
