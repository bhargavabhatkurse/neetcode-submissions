...
vector<int> dp;
    ...
    int numDecodings_bottomup(string s) {
        //dp[i]: //as decoding possible from i to end n-1;
        int n = s.size(); 
        dp.assign(n+1,-1);
        dp[n] = 1; //no of ways of decoding empty string is 1
    ...
    int numDecodings(string s) {
        //dp[i]: //as decoding possible from i to end n-1;
        int n = s.size();
       int dp_next, dp, dp_next_next;
       dp_next = 1; //empty string: 1 way
       dp_next_next = 0; //1 index after the empty out of bound
