...
void bfs(int r_i, int c_i, vector<vector<int>>& heights, vector<vector<bool>>& ocean){
       //multisource bfs
       queue<vector<int>> q;
       q.push({r_i,c_i});
 ...
 vector<vector<int>> pacificAtlantic(vector<vector<int>>& heights) {
        ...
        for(int c = 0; c < cols; c++) { 
           bfs(0,c,heights,pac); //first row
           bfs(rows-1,c,heights,atl); //last row
        }
        for(int r = 0; r < rows; r++) {
           bfs(r,0,heights,pac); //first col
           bfs(r,cols-1,heights,atl); //last col
        }
