...
public:
    ...
    int maxProfit_bottomup(vector<int>& prices) {
     ...
     int maxProfit(vector<int>& prices) {
        ...
        //0 is the least profit
        int n = prices.size();
        vector<int> dpnext2(2,0), dpnext(2,0),dp(2);
        
        int profit; 
        for(int i = n-1;i >= 0; i--){
            for(int j = 0; j < 2; j++) {
                 if(j) { //buying on the ith day
                    profit = max(-prices[i] + dpnext[false], //sell on the next day
                                0 + dpnext[true] //can again buy on the next day
                    );
                }   
                else { //selling on the ith day
                        profit = max(prices[i] + dpnext2[true], //sell on the +2th day
                                0 + dpnext[false]); //(you can think of this as selling and buying again on same day and moving to the next day (so the profit is 0 and essentially you wasted that day))
                }
